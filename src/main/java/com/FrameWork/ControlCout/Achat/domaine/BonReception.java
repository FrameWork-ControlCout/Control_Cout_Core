 ...
 */
@Entity
@Table(name = "Bon_Reception", schema = "param_achat")
@Audited
@AuditTable("Bon_Reception_AUD")
@Getter
@Setter
public class BonReception {
    ...
    @OneToMany(mappedBy = "codeBonReception", cascade = {CascadeType.MERGE, CascadeType.PERSIST}, fetch = FetchType.LAZY)
    @JsonBackReference("listBonReception") // Unique name
    private List<DetailsBonReception> detailsBonReceptions;

    @NotNull
    @Column(name = "Code_Order_Achat", nullable = false, length = 255, columnDefinition = "nvarchar(200)")
